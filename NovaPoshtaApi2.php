...
class NovaPoshtaApi2 {
	 ...
	 * 
	 * @param array $documents Track number
	 * @return mixed
	 */
	function documentsTracking($documens) {
	 ...
	 * @return mixed
	 */
	function getDocumentPrice($document_data) {

        if (empty($document_data['CitySender'])
           || empty($document_data['CityRecipient'])) {
            return array();     
        }
         
        if (empty($document_data['ServiceType'])) {
            $document_data['ServiceType'] = 'WarehouseWarehouse'; 
        }
        
        if (empty($document_data['CargoType'])) {
            $document_data['CargoType'] = 'Cargo'; 
        }
        
        if (empty($document_data['SeatsAbount'])) {
            $document_data['SeatsAbount'] = 1; 
        }
        
        if (empty($document_data['Cost'])) {
            $document_data['Cost'] = 200; 
        }
        
        if (empty($document_data['Weight'])) {
            $document_data['Weight'] = 0.1; 
        }
        
        $response = $this->request('InternetDocument', 'getDocumentPrice', $document_data);
	...
	function newInternetDocument($sender, $recipient, $params) {
        ...
        if ( ! $recipient['CityRecipient']) {
            ...
            if (!empty($recipientCity['data'][0]['Ref'])) {
                $recipient['CityRecipient'] = $recipientCity['data'][0]['Ref']; 
            } elseif (!empty($recipientCity['data'][0][0]['Ref'])) {
        ...
        if (empty($recipient['RecipientAddressName'])) {
            ...
            $street = $this->getStreet($recipient['CityRef'], $recipient['RecipientAddressName']);
            
            $streetRef = $street['data'][0]['Ref'];
            
            $methodProperties = (object) array(
                'CounterpartyRef'   => $recipient['Recipient'],
                'BuildingNumber'    => $recipient['RecipientHouse'],
                'StreetRef'         => $streetRef,
                'Flat'              => $recipient['RecipientFlat'], 
                'Note'              => $recipient['Note']
            );
            
            $address = $this->model('Address')->save($methodProperties);
            $addressRef = $address['data'][0]['Ref']; 
            $recipient['RecipientAddress'] = $addressRef;
        } 
	 ...
	 * @return mixed
	 */
	public function printGetLink($method, $documentRefs, $type) {
...
return new NovaPoshtaApi2('5ca72bfd62c3086833fabc3d7efb65b1', 'ua', false, 'curl'); 
